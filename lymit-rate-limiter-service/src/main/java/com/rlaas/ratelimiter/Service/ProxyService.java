    ...
    private Mono<ResponseEntity<Flux<DataBuffer>>> forwardToBackend(
        ...
        return readySpec
                .exchangeToMono(clientResponse -> {
                    ...
                    // ever subscribes to it. That race is what was producing a 200 with
                    // correct headers but a silently empty body. DataBufferUtils.join fully
                    // drains and copies the backend's response into one independent buffer,
                    // decoupled from the connection, before this function's Mono resolves —
                    // trading pure streaming on this leg for correctness. defaultIfEmpty
                    // covers genuinely empty bodies (e.g. 204, HEAD responses).
